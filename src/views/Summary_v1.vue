    ...
    <div :class="isMobile?'mobile-layout':'normal-layout'">
        <v-card :class="isMobile?'mobile-card':'normal-card'">
            ...
            <v-card v-if="chartOptions.series.length > 0"
                ...
                <div class="chart-div">
                    <highcharts 
                    :class="diffDays > 30?'overflow-chart':'normal-chart'"
...
export default {
    ...
    methods: {
        ...
        async getData () {
            ...
            this.diffDays = ((new Date(this.dates[1]).getTime()) - (new Date(this.dates[0]).getTime())) /(1000*3600*24)
        ...
        filterData () {
            if (this.selectedNames.length > 0){
                console.log("Condition : SelectedNames.length > 0 ");
                if (this.selectedProjects.length > 0){
                    console.log("Condition : SelectedNames&Project.length > 0 ");
                    // select name and project
                    this.createDataNP()
                    this.createSeries()
                    this.createSortMsgAll()
                }else{
                    console.log("Condition : SelectedNames.length > 0 Project.length < 0 ");
                    //select name
                    this.createDataN()
                    this.createSeries()
